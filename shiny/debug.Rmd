...
satcat_meta_active %>% mutate(soi=case_when(X4 == "STARLETTE" ~ TRUE,
                       ...
                       mutate(CAT=1) %>% 
            ggplot() + 
  geom_point(aes(x=as.factor(CAT),y=X9),colour="black",alpha=0.5) + 
  geom_point(data = {. %>% filter(X4=="STARLETTE")}, aes(x=as.factor(CAT),y=X9),colour="red", size=5) +
  geom_label(data = {. %>% filter(X4=="STARLETTE")}, aes(x=as.factor(CAT),y=X9,label=X4),nudge_x=0.12) +
  theme_minimal() + 
